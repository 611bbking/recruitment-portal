...
export const mockLogin = (identifier: string): Candidate => {
  ...
  const candidate: Candidate = existing ?? {
    ...
    gender: '', birth_date: '', school: '', highest_degree: '', major: '', graduation_date: '', preferred_city: '', resume_id: null,
...
export const mockRegister = (name: string, phone: string, email: string): Candidate => {
  const timestamp = now();
  const candidate: Candidate = { candidate_id: makeId('C'), name, phone, email, gender: '', birth_date: '', school: '', highest_degree: '', major: '', graduation_date: '', preferred_city: '', resume_id: null, created_at: timestamp, updated_at: timestamp };
  write(AUTH_KEY, true); write(CANDIDATE_KEY, candidate); return candidate;
};
...
export const updateMockCandidate = (data: ParsedResumeData): Candidate => {
  ...
  const candidate: Candidate = { ...current, name: data.name, phone: data.phone, email: data.email, gender: data.gender, birth_date: data.birth_date, school: data.school, highest_degree: data.degree, major: data.major, graduation_date: data.graduation_date, preferred_city: data.preferred_city, updated_at: now() };
...
export const createMockApplication = (candidateId: string, jobId: string, projectId: string | null): Application => {
  ...
  const timestamp = now();
  const application: Application = { application_id: makeId('A'), candidate_id: candidateId, job_id: jobId, project_id: projectId, status: 'new', owner_id: 'hr-zhang', owner_name: 'HR 张老师', rejection_reason: null, applied_at: timestamp, created_at: timestamp, updated_at: timestamp };
  write(APPLICATIONS_KEY, [...getMockApplications(), application]); return application;
};

export const publicStatusLabels = { new: '已投递', screening: '筛选中', first_interview: '面试中', second_interview: '面试中', offer: '面试中', pre_onboarding: '面试中', onboarded: '已结束', rejected: '已结束', withdrawn: '已结束' } as const;
